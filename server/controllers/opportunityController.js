...
const createOpportunity = async (req, res) => {
    try {
        const { 
            title, description, companyName, opportunityType, 
            location, applicationEmail, tags 
        } = req.body;

        if (!title || !description || !companyName || !opportunityType || !location || !applicationEmail) {
...
const getActiveOpportunities = async (req, res) => {
    try {
        const opportunities = await Opportunity.find({ status: 'active' })
            .sort({ createdAt: -1 })
            .populate('postedBy', 'name email roles');
...
const deleteOpportunity = async (req, res) => {
    try {
        ...
        res.json({ message: 'Opportunity moved to deleted bin', id: req.params.id });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
