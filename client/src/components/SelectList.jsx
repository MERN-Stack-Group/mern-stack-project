  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <input
        ...
        className="w-full bg-white dark:bg-[#111622] border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 p-2 rounded-lg placeholder:text-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors"
        onChange={(e) => {
      ...
      {/* Pop-down Menu */}
      {isOpen && (
        <ul className="absolute left-0 w-full mt-2 overflow-auto border border-slate-200 dark:border-slate-800 rounded-lg shadow-lg top-full max-h-60 bg-white dark:bg-[#111622] z-50">
              ...
              <li
                ...
                className={`p-2 cursor-pointer text-slate-700 dark:text-slate-300 transition-colors ${
                  index === highlightedIndex
                    ? "bg-slate-100 dark:bg-[#161d2b]"
                    : "hover:bg-slate-50 dark:hover:bg-[#161d2b]"
                }`}
                onMouseDown={(e) => {
