...
export default function SelectList({
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {/* Pop-down Menu */}
      {isOpen && (
        <ul className="absolute left-0 w-full mt-2 overflow-auto border border-slate-200 dark:border-slate-800/90 rounded-xl shadow-lg top-full max-h-60 bg-slate-300 dark:bg-[#161d2b] z-50">
          {filteredFaculties.length > 0 ? (
            filteredFaculties.map((option, index) => (
              <li
                key={option}
                className={`px-3.5 py-2 cursor-pointer text-sm text-slate-700 dark:text-slate-300 transition-colors ${
                  index === highlightedIndex
                    ? "bg-slate-400/50 dark:bg-[#1f2838]"
                    : "hover:bg-slate-400/30 dark:hover:bg-[#1f2838]"
                }`}
                onMouseDown={(e) => {
